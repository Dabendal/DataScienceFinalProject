...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='class', 
        title='Success and failure count for ' + entered_site)
        return fig
...
def get_scatter_plot(entered_site,payload_mass):
    lowMass, highMass = payload_mass 
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] > lowMass) & (spacex_df['Payload Mass (kg)'] < highMass)]
    if entered_site == 'ALL' :
        fig = px.scatter(x = filtered_df['Payload Mass (kg)'], y = filtered_df['class'], color = filtered_df['Booster Version Category'], title = 'Success or failure depending on Payload mass and booster version')
        return fig
    else :
        filtered_df2 = filtered_df[filtered_df['Launch Site'] == entered_site]
        fig = px.scatter(x = filtered_df2['Payload Mass (kg)'], y = filtered_df2['class'], color = filtered_df2['Booster Version Category'], title = 'Success or failure for '+ entered_site + 'depending on payload mass and booster version')
        return fig
# Run the app
